...
# Validate the service response for Kafka app
KAFKA_APP_HOST="kafka.example"
KAFKA_APP_PORT=80

echo "Validating the service response for Kafka app..."
RESPONSE=$(curl --resolve "${KAFKA_APP_HOST}:${KAFKA_APP_PORT}:127.0.0.1" -s -o /dev/null -w "%{http_code}" "http://${KAFKA_APP_HOST}:${KAFKA_APP_PORT}")
...
if [ "$RESPONSE" -eq 200 ]; then
    ...
    if grep -q "127.0.0.1 ${KAFKA_APP_HOST}" /etc/hosts; then
        echo "/etc/hosts already contains the entry."
    else
        echo "127.0.0.1 ${KAFKA_APP_HOST}" | sudo tee -a /etc/hosts > /dev/null
        echo "/etc/hosts updated successfully."
    fi

    # Open in default browser
    echo "Opening ${KAFKA_APP_HOST}:${KAFKA_APP_PORT} in the default browser..."
    open_browser "http://${KAFKA_APP_HOST}"
